    ...
    unless value.blank?
      ...
      CARD_TYPES.each { |type|
        if value =~ type[:pattern]
          ...
          if result
            record.credit_card_type = type[:name] if record.is_a? User
            record.type_name = type[:name] if record.is_a? Card
          end

        end
      }
      record.errors[attribute] << I18n.t('model.errors.custom.credit_card_length') unless result
    end
  end
end