...
module ApplicationHelper
  ...
  def self.pdf?(request)
    request.env['REQUEST_PATH'].include? '.pdf' rescue nil
  end

  def self.csv?(request)
    (request.env['REQUEST_PATH'].include?('.csv') || request.env['REQUEST_PATH'].include?('.cvs'))rescue nil
  end
  ...
  MONTHS = ['Month', '01 - January', '02 - February', '03 - March', '04 - April', '05 - May', '06 - June', '07 - July', '08 - August', '09 - September', '10 - October', '11 - November', '12 - December']
  def months
    MONTHS.each_with_index.map {|p, i| [p, i]}
  end

  def years
    [%w(Year 0)] + (Range.new(Time.now.year, Time.now.year+Configurations[:credit_card_year_range])).to_a.each.map {|p| [p, p]}
  end
